...
static ssize_t lcdoff_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
...
static ssize_t cabc_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	if(is_lcd(OPPO18136_HIMAX_HX83112A_1080_2340_VOD_PANEL) || is_lcd(OPPO18321_DPT_NT36672A_1080_2340_VOD_PANEL))
	{
		return 0;
	}
	pr_debug(KERN_INFO "get cabc mode = %d\n",cabc_mode);
...
static ssize_t closebl_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	pr_debug(KERN_INFO "get closebl flag = %d\n",lcd_closebl_flag);
	return sprintf(buf, "%d\n", lcd_closebl_flag);
}
